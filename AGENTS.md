...
**動手前必讀 [docs/HANDOFF.md](docs/HANDOFF.md)**（完整架構地圖、不變式、測試與發佈流程）。本檔只列最關鍵的規則；`CLAUDE.md` 是本檔的鏡像（給 Claude Code），兩檔內容需同步修改。
...
3. 線稿轉換（`toLineArt`）必須輸出二值圖（線 34、底 255），油漆桶填色依賴這點。
